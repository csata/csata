...
class User::RegisterController < ApplicationController
  ...
  def update_chair_select
    if params[:id] == "0"
      ...
      render :partial => "chair", :locals => {:chairs => Faculty.find_by_id(params[:id]).chairs}
    end    
  end
  ...
  def validate_step1_no_xhr?
    unless validate_step1?
      ...
      elsif session[:password] !~ /^([a-z0-9])+$/i
        flash.now[:password] = "пароль должен состоять только из латинских букв и цифр"
  ...
  def validate_step3?
    session[:phone] == "" || session[:phone] =~ /^[0-9]{11}$/i
  end
